...
tab <- tibble::tribble(
  ...
  mutate(across(-Felicidade, ~format(.x, decimal.mark = ",", nsmall = 1)))

html_tab <- kable(
  tab, format = "html", align = c("l","r","r"),
  table.attr = 'class="tbl-book"'
)
...
<div class="table-wrap">
```{r tbl_conjunta, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
...
tab2 <- tibble::tribble(
  ...
  mutate(
    Felicidade = ifelse(
      Felicidade == "Total",
      cell_spec("Total", "html", bold = TRUE),  # negrito só neste *célula*
      Felicidade
    )
  )
...
html_tab2 <- kable(
  tab2, format = "html",
  align = c("l", rep("c", 5), "r"),
  table.attr = 'class="tbl-book"',  # mantém seu estilo
  escape = F                    # necessário por causa do cell_spec()
) |>
  add_header_above(c(" " = 1, "Classe social" = 5, " " = 1), bold = F) |>
  row_spec(0, bold = F) |>
  kable_styling(font_size = 15)      # opcional: tamanho da fonte desta tabela
